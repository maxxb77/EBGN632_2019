...
$if not set tabledecision $setglobal tabledecision on
...
$ifthen.tabletemp '%tabledecision%'=='off'
...
rep("profit") = sum((i,e),p(i,e) * X.l(i,e))
             -sum((j,s),c(j,s) * Y.l(j,s));

rep_unit(i,e) = x.l(i,e);

rep_supplier(j,s) = y.l(j,s);

execute_unload 'alldat_%tabledecision%.gdx'
